...
Aluno LoginController::loginAluno(string email, string senha) const {
    // 1. Busca o Aluno pelo email. O Service retorna o objeto por valor (dentro
    // de optional).
    ...
    // 3. CRÍTICO: Cria o objeto no HEAP e o armazena em um unique_ptr<Usuario>.
    // O objeto 'aluno' é movido (seus dados são transferidos) para a nova
    // memória no heap. Isso é feito para: A) Garantir que o objeto sobreviva ao
    // fim desta função. B) Habilitar o polimorfismo (upcast implícito de Aluno
    // para Usuario).
    ...
    // 5. Sucesso: Transfere a propriedade (Ownership) para o SessionManager.
    // O std::move transfere o unique_ptr. Após esta linha, a SessionManager é a
    // única responsável por deletar o objeto.
...
Professor LoginController::loginProfessor(string email, string senha) const {
    // 1. Busca o Professor.
    optional<Professor> opt_professor = professorService.getOneByEmail(email);
